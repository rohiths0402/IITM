...
void (*set_value_func)(const char*, const char*);
const char* (*get_value_func)(const char*);
    ...
    while (1) {
        ...
        if (strncmp(buf, "SET ", 4) == 0) {
            ...
            if (sscanf(buf + 4, "%63s", key) == 1) {
                const char* v = get_value_func(key);
...
int main(void) {
    void *handle = dlopen("./libkvstore.so", RTLD_NOW);
    if (!handle) {
        fprintf(stderr, "dlopen failed: %s\n", dlerror());
        exit(EXIT_FAILURE);
    }

    *(void**)(&set_value_func) = dlsym(handle, "set_value");
    *(void**)(&get_value_func) = dlsym(handle, "get_value");
    if (!set_value_func || !get_value_func) {
